...
Nexus Agent determines if a query requires internet access and, if so, performs searches using SearxNG or DuckDuckGo, scrapes, parses, and stores the data in a vector database for more informed LLM responses.

## Why this project?

With the rise of local LLMs, their capabilities are often limited to pre-existing knowledge without real-time internet access. Nexus Agent solves this problem by enabling LLMs to fetch up-to-date information, enhancing their accuracy and utility in providing answers to user queries.
...
First, clone the Nexus Agent repository and install the necessary dependencies:

```bash
git clone https://github.com/ProfessionalGovind/Nexus-Agent.git
